...
class _AddQuestionScreenState extends State<AddQuestionScreen> {
  ...
  Future<void> _fetchCategories() async {
    ...
    if (_questionCategories.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
              'No categories available. Please create a new category using +.'),
  ...
  void _addQuestion() async {
    if (_textController.text.isNotEmpty &&
      ...
      if (_selectedType == 'Multiple Choice') {
        ...
        if (!_correctChoices.contains(true)) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                  'Please select at least one correct choice for multiple-choice questions.'),
              behavior: SnackBarBehavior.floating,
              margin: EdgeInsets.only(bottom: 80.0, left: 16.0, right: 16.0),
            ),
          );
      ...
      int questionId = await DatabaseHelper().insertQuestion({
        ...
        'essay_spaces': _selectedType == 'Essay' ? _essaySpaces : null,
        // Save essay spaces if type is 'Essay'
      });
      ...
      Navigator.of(context)
          .pop(); // Close the screen after adding the question.
    } else {
            ...
            TextButton(
              onPressed: () async {
                if (categoryNameController.text.isNotEmpty) {
                  ...
                  int? id = await DatabaseHelper().getCategoryId(
  ...
  void _addCategory(String categoryName) async {
    await DatabaseHelper().insertCategory(
        {'name': categoryName, 'subject_id': widget.subjectId});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Row(
          children: [
            Text('Add New Question'), // Title of the app bar
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                  labelText: 'Question Text'), // Text field for entering question text
            ),
            DropdownButtonFormField<String>(
              ...
              onChanged: (value) {
                setState(() {
                  _selectedType = value; // Update selected question type
                });
              },
              decoration: const InputDecoration(
                  labelText: 'Question Type'), // Dropdown for selecting question type
            ),
            Row(
              children: [
                Expanded(
                  child: DropdownButtonFormField<int>(
                    ...
                    decoration: const InputDecoration(
                        labelText: 'Question Category'), // Dropdown for selecting question category
                  ),
                ),
                IconButton(
            ...
            if (_selectedType == 'Essay')
              DropdownButtonFormField<int>(
                ...
                onChanged: (value) {
                  setState(() {
                    _essaySpaces =
                    value!; // Update number of lines for essay questions
                  });
                },
                decoration: const InputDecoration(
                    labelText: 'Number of lines for Essay'), // Dropdown for selecting number of lines for essay questions
              ),
            if (_selectedType == 'Multiple Choice')
              Column(
                children: [
                      ...
                      children: [
                        Expanded(
                          child: TextField(
                            ...
                            decoration: InputDecoration(labelText: 'Choice ${i +
                                1}'), // Text field for entering multiple-choice options
                          ),
                        ),
                        Checkbox(
                          ...
                          onChanged: (value) {
                            setState(() {
                              if (_choiceControllers[i].text.isNotEmpty) {
                                _correctChoices[i] =
                                value!; // Update correct choices for multiple-choice options
                              } else {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text(
                                        'Correct choice cannot be blank.'),
                                    behavior: SnackBarBehavior.floating,
                                    margin: EdgeInsets.only(
                                        bottom: 80.0, left: 16.0, right: 16.0),
                                  ),
                                );
                              }
                            });
                  ...
                  ElevatedButton(
                    ...
                    child: const Text(
                        'Add Choice'), // Button to add additional choices for multiple-choice questions
                  ),
                ],
              ),
            if (_selectedType == 'Fill in the Blank')
              TextField(
                ...
                decoration: const InputDecoration(
                    labelText: 'Correct Answer'), // Text field for entering correct answer for fill in the blank questions
              ),
            const SizedBox(height: 100),
            // Add spacing to avoid overlap with the button
          ],
        ),
      ),
      bottomNavigationBar: Padding(
        ...
        child: ElevatedButton(
          ...
          child: const Text('Save'), // Button to save the question
        ),
      ),
    );
  }
